...
clearOldRoot() {
    ...
    if [ -f $output.sig ]; then
        log "Removing old root filesystem SquashFS signature";
        rm -f $output.sig
    fi
}
...
installPackages() {
    ...
    cp mkinitcpio.conf $rootdir/etc/mkinitcpio.conf
}
...
#    mksquashfs $rootdir $output -comp zstd -Xcompression-level 22
    mksquashfs $rootdir $output -comp xz
}
...
log "SquashFS generation complete. Don't forget to run sign.sh to sign the image. Now you can use makepkg to build the recovery image package."
